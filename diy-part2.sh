...
cp -r /home/runner/work/Actions-OpenWrt/Actions-OpenWrt/package/* /home/runner/work/Actions-OpenWrt/Actions-OpenWrt/openwrt/package/
cp -r /home/runner/work/Actions-OpenWrt/Actions-OpenWrt/lede/feeds/packages/net/vlmcsd/ /home/runner/work/Actions-OpenWrt/Actions-OpenWrt/openwrt/package/
...
if [ $TVH == true ]; then CONFIG_FILE=$CONFIG_FILE0; cp $CONFIG_FILE .config; cat myconfig/config.tvh.ffmpeg >>.config; else CONFIG_FILE=$CONFIG_FILE2 && cp $CONFIG_FILE .config; fi
echo `ls -alt .config`;echo `ls -alt myconfig/config.$ROUTER_MODEL.?`;cat .config|grep tvheadend
...
#exit 0
[ $CPU_MULTI_CORE == true ] && cat myconfig/base.update >> feeds/luci/modules/luci-base/po/zh_Hant/base.po && cp $UPDATE_MULTI_CORE_FILE1 feeds/luci/modules/luci-base/luasrc/sys.lua && cp $UPDATE_MULTI_CORE_FILE2 feeds/luci/modules/luci-mod-status/htdocs/luci-static/resources/view/status/processes.js && sed -i 's/processes.js:72/processes.js:74/g' feeds/luci/modules/luci-base/po/zh_Hant/base.po  && sed -i 's/processes.js:73/processes.js:75/g' feeds/luci/modules/luci-base/po/zh_Hant/base.po
exit 0
